...
def gerar_graficos_resultado_liquido_turma(resultados:pd.DataFrame=None, nome_fig:str=None):
    ...
    resultados = resultados.sort_values(by='IDTURMA').reset_index(drop=True)
    resultados = resultados.sort_values(by='IDTURMA').reset_index()
    
    #print(resultados['index'])

    plt.figure(figsize=(8, 5))
    plt.plot(resultados['index'], resultados['ResultadoLiquido'], color='blue')
    ...
    plt.title('Resultado Líquido por Turma - Backtracking')
    plt.xlabel('Turmas')
...
def gerar_graficos_carga_horario_professor(carga_horaria_professor:pd.DataFrame=None, nome_fig:str=None):
    ...
    plt.title('Carga Horária por Professor - Backtracking')
...
def gerar_graficos_quantidade_turmas_professor(quantidade_turmas_professor:pd.DataFrame=None, nome_fig:str=None):
    ...
    plt.title('Quantidade de Turmas por Professor - Backtracking')
